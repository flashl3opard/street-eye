...
export const DEFAULT_DATA_URL = process.env.NEXT_PUBLIC_HARDWARE_URL || 'http://172.20.10.2/data';
...
export const LDR_BULB_THRESHOLD = 30;
...
export function inferStatus(current, ldr) {
  const hasCurrent = current > CURRENT_ON_THRESHOLD;  // relay energised
  const bulbLit    = ldr    > LDR_BULB_THRESHOLD;     // bulb emitting light

  if (hasCurrent  && bulbLit)  return 'ok';      // Power ON, light ON  → Working
  if (hasCurrent  && !bulbLit) return 'fault';   // Power ON, light OFF → Fused bulb
  if (!hasCurrent && bulbLit)  return 'warn';    // Power OFF, light ON → Wastage
  return 'standby';                              // Power OFF, light OFF → Lamp correctly off
}
